			...
			union lge_power_propval *val)
{
	...
	case LGE_POWER_PROP_BATT_CAPACITY_MIN:
		val->intval = battery_id->batt_capacity_min;
		break;
...
static int lge_battery_id_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "lge,batt_capacity_min",
						&battery_id->batt_capacity_min);
	if (ret < 0) {
		pr_err("battery capacity cannot be get\n");
		battery_id->batt_capacity_min = 3185;
	}
