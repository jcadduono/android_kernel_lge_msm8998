...
unsigned int cancun_dbot_pa0_temp;
unsigned int cancun_dbot_pa1_temp;
unsigned int cancun_dbot_quiet_temp;
#endif
...
static int get_vts(struct lge_monitor_thermal_data *monitor_dd)
{
	if (!monitor_dd)
		return -ENODEV;
...
static void _poll_monitor(struct lge_monitor_thermal_data *monitor_dd)
{
	...
	if (!IS_ERR(monitor_dd->vadc_dev)) {
		...
		rc = qpnp_vadc_read(monitor_dd->vadc_dev,
					0x4f, &result);  // amux3 pa1 100k
		if (rc)
			pr_err("VADC read error with %d\n", rc);
		else
			monitor_dd->last_pa0_temp = (unsigned int)result.physical;

		rc = qpnp_vadc_read(monitor_dd->vadc_dev,
					0x50, &result);  // amux4 pa2 100k
		if (rc)
			pr_err("VADC read error with %d\n", rc);
		else
			monitor_dd->last_pa1_temp = (unsigned int)result.physical;

		rc = qpnp_vadc_read(monitor_dd->vadc_dev,
					0x51, &result);  // amux6 quiet 100k
		if (rc)
			pr_err("VADC read error with %d\n", rc);
		else
			monitor_dd->last_quiet_temp = (unsigned int)result.physical;

		rc = get_vts(monitor_dd);
		if (rc < 0) {
			pr_err("does not exist VTS node, calculation manually\n");
			/* TODO : after analysis about Lucy's VTS
			 * temporary applied combination of xo & quiet therm */
			monitor_dd->last_vts_temp = 333;
			/*
		...
		cancun_dbot_pa0_temp = monitor_dd->last_pa0_temp;
		cancun_dbot_pa1_temp = monitor_dd->last_pa1_temp;
		cancun_dbot_quiet_temp = monitor_dd->last_quiet_temp;
		cancun_dbot_vts_temp = monitor_dd->last_vts_temp;
#endif
		pr_err("[TM] XO,%3d,PA0,%3d,PA1,%3d,QUIET,%3d,VTS,%u\n",
