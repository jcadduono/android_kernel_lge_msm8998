/*
 * stub_tx.c - sending mausb packets
 * - This file is derived form /drivers/usb/usbip/stub_tx.c
 *
...
static int stub_send_ret_submit(struct stub_device *sdev)
{
	...
	while ((pal = dequeue_from_mausb_pal_tx(sdev)) != NULL) {
		...
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"header->u.non_iso_hdr.reqid_seqno: %d",header->u.non_iso_hdr.reqid_seqno);
		...
		header->u.non_iso_hdr.remsize_credit =
			sizeof(struct mausb_header) + pal->urb->actual_length;

		//header->u.non_iso_hdr.remsize_credit = txsize;

		//DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,
				//"send msg..remsize_credit..%zu\n",header->u.non_iso_hdr.remsize_credit);
		if(pal->urb->actual_length < 128)
		print_hex_dump(KERN_INFO, "PAYLOAD:", DUMP_PREFIX_ADDRESS, 16, 1,
							pal->urb->transfer_buffer, pal->urb->actual_length, false);
		printk(KERN_INFO"\n");
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,
				"send msg..txsize..%zu\n",txsize);
		...
		if (ret != txsize) {
			...
			DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,
					"sendmsg success, retval %d for %zd\n",
				ret, txsize);
		} else {
			DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,
					"sendmsg failed!, retval %d for %zd\n",
	...
	DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_MAIN,
			"Time taken for transaction: %u  milli seconds\n",
			(unsigned int)(actual_time));
#endif
	return total_size;
}

static struct stub_mausb_pal *dequeue_from_mausb_unlink_tx(
...
static int stub_send_ret_unlink(struct stub_device *sdev)
{
	...
	while ((mausb_unlink = dequeue_from_mausb_unlink_tx(sdev)) != NULL) {
		...
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"---> %s\n",__func__);
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"\n unlink_pal address : %p\n",mausb_unlink);
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"hdr->u.non_iso_hdr.reqid_seqno %u\n", hdr->u.non_iso_hdr.reqid_seqno);
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"mausb_unlink->seqnum %u\n", mausb_unlink->seqnum);
		...
		hdr->base.length = sizeof(struct mausb_header);
	...
	list_for_each_entry_safe(mausb_unlink, mausb_tmp,
			&sdev->mausb_unlink_free, list) {
