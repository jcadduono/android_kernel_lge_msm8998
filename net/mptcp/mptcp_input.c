...
static int mptcp_validate_mapping(struct sock *sk, struct sk_buff *skb)
{
	...
	// Fix WBT #183665
	if (tmp != NULL && before(TCP_SKB_CB(tmp)->seq, tp->mptcp->map_subseq) &&
	 ...
	 */
	 // Fix WBT #183666
	if (skb_peek(&sk->sk_receive_queue) != NULL && before(TCP_SKB_CB(skb_peek(&sk->sk_receive_queue))->seq, tp->mptcp->map_subseq)) {
